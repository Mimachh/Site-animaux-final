...
<div class="pb-5">
    <div class="grid grid-cols-3 gap-4 mx-10 my-10 border rounded shadow hover:shadow-lg"">
            ...
            <div class="md:col-span-2 col-span-3 pl-5">
                ...
                @if($annonce->start_watch && $annonce->end_watch !== null)
                    <div class="flex text-md">
                        ...
                        <p class="text-gray-600 pb-3 pl-4">
                            Disponible du : <span class=" text-gray-800">{{date('d/m/Y', strtotime($annonce->start_watch))}}</span>  
                ...
                @if($annonce->ville_id !== null)
                    <div class="flex justify-center">
                        ...
                        <p class="text-gray-600 pl-4 pb-3">Ville : <span class="text-gray-800"> {{ $annonce->ville->ville_nom }}</span></p>    
                    </div>
                @endif
                <!--Fin ville -->
                <!-- Type de garde -->
                <p class="text-gray-600 pb-3">Type de garde :               
                    <span class="text-gray-800">{{$annonce->garde->garde}}</span>       
                </p>
                <!-- Fin type de garde -->
                <p class="text-gray-600 pb-2">Animaux gardés :
                ...
                <label for="description" class="my-1 font-medium text-gray-600 mt-6">Quelques mots sur moi :</label> 
                <p class="text-gray-800 mb-5" id="description" name="description">{{ $annonce->description }}</p>
                                                        
                <div class="">
                    <label for="prix" class="inline-block my-1 text-md font-medium text-gray-600 mt-5"> Mon tarif :</label>
                    <p class="inline-block my-1 font-medium text-gray-900 pb-3" id="prix" name="prix">{{ number_format($annonce->price, 2, ',', '') }} €/jour</p>
                </div> 
            </div>
        ...
        @foreach($animals as $animal)
        <div class="md:col-span-2 col-span-3 bg-red-600">
                <h1>{{ $animal->animal_name }}</h1>
                <p> {{ $animal->personnality }} </p>
        ...
        <div class="bg-red-600 col-span-3 md:col-span-1">
            <div class="mx-2 my-2 aspect-w-1 aspect-h-1 overflow-hidden rounded-lg bg-gray-200 xl:aspect-w-7 xl:aspect-h-8 shadow-xl">
                <img class="object-cover object-center group-hover:opacity-75" src="{{ asset('storage/animals_photos/' . $animal->photo) }}">
            </div>
        </div>
        @endforeach
</div>
